...
export interface Env {
  ...
  /** Cloudflare KV namespace used for the 30s coalescing window
   *  (Phase C). Markers are keyed by (user_id, portfolio_id,
   *  data_type) and expire automatically — no manual eviction needed.
   *  Bound in wrangler.jsonc; the namespace ID is generated by
   *  `wrangler kv namespace create PUSH_COALESCE` and pasted in. */
  PUSH_COALESCE: KVNamespace;

  /** Sentry DSN for the same project the iOS + web apps use. Events
   *  are tagged `service: "push-worker"` so they're filterable. */
  SENTRY_DSN: string;

  /** Per-user daily push cap (Phase E). Counted against rows in
   *  `push_log` with outcome IN ('delivered','apns_error',
   *  'token_dead') in the last 24h. Set <= 0 to disable. Default
   *  applied in `rateLimit.ts` if the var is missing. Stored as a
   *  Worker var (not a secret) so it can be tweaked without rotating
   *  anything sensitive. */
